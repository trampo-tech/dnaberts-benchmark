...
agg = (
    ...
    .agg(
        mcc_mean=("test_mcc", "mean"),
        mcc_std =("test_mcc", "std"),
...
# =====================================================
# VISÃO 1 — MACRO: MCC médio vs Tempo (scatter)
# =====================================================
...
ax.set_ylabel("MCC Médio nas Tarefas GUE",              fontsize=8)
...
ax.text(0.01, 0.97, "Tamanho da bolha ∝ parâmetros do modelo",
        transform=ax.transAxes, fontsize=5.5, va="top", color="gray")

plt.tight_layout()
plt.savefig("reports/figures/visao1_macro.png", dpi=300, bbox_inches="tight")
plt.savefig("reports/figures/visao1_macro.pdf", bbox_inches="tight")
plt.close()
print("✓ Visão 1 — Macro salva")
...
for model in model_order:
    for task in tasks_gue:
        ...
        if model == "EVO2":
            ft = sub[sub["training_type"].isin(["Full FT", "LoRA"])].sort_values("mcc_mean", ascending=False).head(1)
        else:
            ft = sub[sub["training_type"] == ft_priority[model]]

        if not no_ft.empty:
            records.append({"model": model, "task": task, "training": "Sem FT",  "mcc": no_ft.iloc[0]["mcc_mean"]})
        if not ft.empty:
            records.append({"model": model, "task": task, "training": "Com FT", "mcc": ft.iloc[0]["mcc_mean"]})

strat_df  = pd.DataFrame(records)
strat_avg = strat_df.groupby(["model", "training"])["mcc"].mean().reset_index()
...
for i, (label, hatch) in enumerate(zip(labels, hatches)):
    ...
    for model in model_order:
        ...
        vals.append(row.iloc[0]["mcc"] if not row.empty else 0)
...
for i, model in enumerate(model_order):
    ...
    gain = r1.iloc[0]["mcc"] - r0.iloc[0]["mcc"]
    ax.annotate(
        f"+{gain:.2f}" if gain >= 0 else f"{gain:.2f}",
        xy=(x[i], max(r0.iloc[0]["mcc"], r1.iloc[0]["mcc"]) + 0.07),
...
ax.set_ylabel("MCC Médio nas Tarefas GUE", fontsize=8)
ax.set_title("Impacto do Fine-Tuning — Quanto o FT ajuda?", fontsize=9, fontweight="bold")
ax.set_ylim(0, strat_avg["mcc"].max() + 0.25)
...
plt.tight_layout()
plt.savefig("reports/figures/visao2_estrategia.png", dpi=300, bbox_inches="tight")
plt.savefig("reports/figures/visao2_estrategia.pdf", bbox_inches="tight")
plt.close()
print("✓ Visão 2 — Estratégia salva")
...
for i, tt in enumerate(["Sem FT", "Full FT", "LoRA"]):
    ...
    for tl in tasks_sorted:
        ...
        if row.empty:
            ...
            vals.append(row.iloc[0]["mcc_mean"])
            errs.append(row.iloc[0]["mcc_std"] if not pd.isna(row.iloc[0]["mcc_std"]) else 0)
...
ax.set_title("EVO2 — Sem FT vs Full FT vs LoRA", fontsize=9, fontweight="bold")
ax.set_ylim(0, evo2["mcc_mean"].max() + evo2["mcc_std"].max() + 0.15)
ax.tick_params(axis="y", labelsize=7)
ax.yaxis.grid(True, linewidth=0.5, linestyle="--", alpha=0.6)
ax.set_axisbelow(True)
...
plt.tight_layout()
plt.savefig("reports/figures/visao3_micro_evo2.png", dpi=300, bbox_inches="tight")
plt.savefig("reports/figures/visao3_micro_evo2.pdf", bbox_inches="tight")
plt.close()
print("✓ Visão 3 — Micro EVO2 salva")
...
for model in model_order:
    for task in tasks_gue:
        sub = agg[(agg["base_model"] == model) & (agg["task"] == task)]
        if sub.empty:
            heat_records.append({"model": model, "task": task_label_map.get(task, task), "mcc": np.nan, "training": ""})
        else:
            best = sub.sort_values("mcc_mean", ascending=False).iloc[0]
            heat_records.append({
                "model": model, "task": task_label_map.get(task, task),
                "mcc": best["mcc_mean"], "training": best["training_type"],
            })

heat_df    = pd.DataFrame(heat_records)
heat_pivot = heat_df.pivot(index="task", columns="model", values="mcc") \
...
sns.heatmap(
    ...
    annot_kws={"size": 6.5}, cbar_kws={"label": "MCC", "shrink": 0.8},
    vmin=0, vmax=1,
)
ax.set_title("Melhor MCC por Modelo × Tarefa GUE (todas as estratégias)", fontsize=9, fontweight="bold")
...
plt.tight_layout()
plt.savefig("reports/figures/visao4_geral_heatmap.png", dpi=300, bbox_inches="tight")
plt.savefig("reports/figures/visao4_geral_heatmap.pdf", bbox_inches="tight")
plt.close()
print("✓ Visão 4 — Heatmap GUE salva")
...
plt.savefig("reports/figures/visao5_bend.pdf", bbox_inches="tight")
plt.close()
print("✓ Visão 5 — BEND salva")

print("\nTodos os arquivos gerados em reports/figures/")